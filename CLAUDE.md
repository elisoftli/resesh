...
- **`providers/claude.ts`** — Claude Code provider. Reads `~/.claude/projects/{encoded-path}/{sessionId}.jsonl`. On Windows, also scans the default WSL distro (gated behind the `includeWsl` preference). Uses `ensureWslScan()` for deduplicated scanning with in-memory cache. Exports `claudeProvider`.
- **`providers/wsl.ts`** — Shared WSL infrastructure used by both providers. `getDefaultWslDistro()` (finds the default distro via `wsl -l`, cached as a shared promise), `wslEnabled()` (checks `includeWsl` preference), `execWsl()` (runs a shell script inside WSL, returns stdout), `streamWslFiles()` (streams JSONL file contents via `\tFILE\t` header protocol, supports `AbortSignal` to kill the process on cancel). Windows-only; all functions return empty on other platforms.
- **`providers/codex.ts`** — Codex CLI provider. Reads `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`. On Windows, also scans the default WSL distro (gated behind `includeWsl` preference). Uses `ensureWslScan()` for deduplicated scanning with in-memory cache. Exports `codexProvider`.
...
- On Windows, both providers scan the default WSL distro's session directories in parallel with native sessions. WSL scanning is opt-in via the `includeWsl` preference. Each provider uses `ensureWslScan(distro)` — a shared function that runs exactly one `streamWslFiles` scan (via `wsl exec`), caches all session data (metadata + messages) in a module-level `Map`, and deduplicates concurrent callers via an in-flight promise. Both `discoverProjects` and `searchSessions` call `ensureWslScan`; project info is derived from the cache (no separate discover WSL process). After the first scan, all subsequent searches (keystrokes) run purely in-memory against the cache — zero WSL processes. `streamWslFiles` accepts an `AbortSignal` that kills the WSL process on cancel. WSL project dirs are keyed as `wsl:<distro>:<dir>` to avoid collisions. `SessionInfo.wslDistro` tracks origin for resume commands and path conversion.
